...
describe Memberships::JoinZusatzsektionMailer do
  ...
  context "zusatzsektionsektion requiring approval" do
    ...
    it "sends confirmation email" do
      ...
      expect(fees).to have_css("tr:nth-of-type(1)", text: "EUR 42.00\r\njährlicher Beitrag")
      expect(fees).to have_css("tr:nth-of-type(2)", text: "EUR 21.00\r\n- 50% Rabatt auf den jährlichen Beitrag")
      expect(fees).not_to have_css("tr:nth-of-type(3)")
      expect(fees).to have_css("tfoot tr", text: "EUR 21.00\r\nTotal erstmalig")
    end

    it "includes abroad fees for person living abroad" do
      person.update!(country: "BE")
      expect(fees).to have_css("tr:nth-of-type(3)", text: "EUR 6.50\r\n+ Gebühren Ausland")
    end
  ...
  context "sektion not requiring approval" do
    ...
    it "sends confirmation email" do
      ...
      expect(body).to include(person_path(person))
      expect(fees).to have_css("tr:nth-of-type(1)", text: "EUR 84.00\r\njährlicher Beitrag")
      expect(fees).to have_css("tr:nth-of-type(2)", text: "EUR 42.00\r\n- 50% Rabatt auf den jährlichen Beitrag")
      expect(fees).not_to have_css("tr:nth-of-type(3)")
      expect(fees).to have_css("tfoot tr", text: "EUR 42.00\r\nTotal erstmalig")
    end
